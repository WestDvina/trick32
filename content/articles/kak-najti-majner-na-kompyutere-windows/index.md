...
<!-- 📷 СКРИН 1 | файл: simptom-nagruzka.webp
     Показать: Диспетчер задач, вкладка «Производительность» (или Монитор ресурсов).
     Что видно: график ЦП/ГП держится на 70–100%, когда открыт только рабочий стол.
     Отметить: рамкой обвести график загрузки; подпись «в простое, программы закрыты».
     Заменить эту строку на: ![Загрузка процессора 100% в простое](simptom-nagruzka.webp) -->
...
<!-- 📷 СКРИН 2 | файл: process-explorer-put-podpis.webp
     Показать: Process Explorer, включён режим View → Show Process Tree.
     Что видно: столбцы Process, CPU, Private Bytes, Company Name, Description, Path.
     Отметить: стрелками/рамками — (1) поддельный svchost.exe с путём из C:\Users\...\AppData\...,
                (2) пустой или чужой издатель (Company Name), (3) родитель не services.exe.
     Заменить эту строку на: ![Поддельный svchost.exe с путём из AppData](process-explorer-put-podpis.webp) -->
...
<!-- 📷 СКРИН 3 | файл: planirovshchik-zadacha-temp.webp
     Показать: Планировщик заданий (taskschd.msc), список активных задач.
     Что видно: задача с непонятным именем, во вкладке «Действия» запускается .exe из %TEMP%, %AppData% или C:\ProgramData.
     Отметить: рамкой — имя задачи и путь к исполняемому файлу.
     Заменить эту строку на: ![Задача в Планировщике запускает exe из TEMP](planirovshchik-zadacha-temp.webp) -->
...
<!-- 📷 СКРИН 4 | файл: netstat-ano-soedineniya.webp
     Показать: окно командной строки с выводом netstat -ano.
     Что видно: строка ESTABLISHED и столбец PID; несколько соединений на один внешний IP, порт из списка 3333/4444/5555/14444.
     Отметить: рамками — внешний адрес:порт и PID в правом столбце.
     Заменить эту строку на: ![netstat -ano: соединения процесса на майнинг-пул](netstat-ano-soedineniya.webp) -->
...
Всё просто: откройте Диспетчер задач, вкладку «Подробности», отсортируйте по ЦП. Если одна вкладка `chrome.exe` или `firefox.exe` грузит ядро на 100% — сайт встроил WebAssembly-майнер. Закрыли вкладку — нагрузка исчезла.

<!-- 📷 СКРИН 5 | файл: brauzer-vkladka-100.webp
     Показать: Диспетчер задач, вкладка «Подробности», сортировка по столбцу «ЦП» (по убыванию).
     Что видно: процесс chrome.exe/firefox.exe с одной-двумя вкладками держит 100% ядра.
     Отметить: рамкой — строку процесса и высокую загрузку ЦП; подпись «одна вкладка».
     Заменить эту строку на: ![Одна вкладка браузера грузит ядро на 100%](brauzer-vkladka-100.webp) -->
...
<!-- 📷 СКРИН 6 | файл: kvrt-skanirovanie.webp
     Показать: окно Kaspersky Virus Removal Tool (или Dr.Web CureIt!) во время проверки.
     Что видно: идёт сканирование, прогресс-бар, список найденных угроз (или «угроз не найдено»).
     Отметить: рамкой — результат проверки; без реальных личных данных/путей пользователя.
     Заменить эту строку на: ![Проверка системы портативным сканером](kvrt-skanirovanie.webp) -->
