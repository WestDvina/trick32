...
Windows 11 научилась принимать отказ. В навязчивом промпте OneDrive появилась кнопка **Opt out of backup** - нажать один раз и больше не откладывать решение на 3 дня.

Раньше такого выбора не было. Система предлагала только **Продолжить** или **Напомнить через 3 дня**. Отказаться по-честному было нельзя.
...
- **Continue** - включить резервное копирование
- **Remind me in 3 days** - отложить

Стало:

- **Continue** - включить
- **Opt out of backup** - отказаться насовсем
...
*Новый промпт OneDrive: синяя кнопка Continue и малозаметная ссылка Opt out of backup. Скриншот: Windows Latest via Pureinfotech*

Разница принципиальная. **Remind me in 3 days** не означал «нет». Он означал «спроси меня снова». **Opt out** означает «я не хочу, не спрашивай».
...
На этапе OOBE появлялся экран **Backup is recommended** с кнопками **Continue** и **Skip for now**.

**Skip for now** тоже не был отказом. Система запоминала его как «пока пропустим», а через время показывала промпт снова. Получался цикл: пропустил - напомнили - пропустил - напомнили.
...
Если хотите отказаться - ищите именно текст **Opt out of backup** внизу или сбоку от основной кнопки. Не путайте с закрытием окна крестиком - крестик тоже часто работает как «напомнить позже».
...
Включить бэкап позже можно вручную. Тот же путь: **Параметры - Учётные записи - Архивация Windows - OneDrive**. Или через иконку OneDrive в трее - **Параметры - Синхронизация и резервное копирование - Управление резервным копированием**.
...
- у вас медленный интернет или лимитный тариф
...
Microsoft наконец разделила «не сейчас» и «не хочу». Кнопка **Opt out of backup** в Windows 11 25H2 закрывает старую проблему: раньше отказаться было нельзя, можно было только откладывать.

Дизайн по-прежнему подталкивает к **Continue**. Но теперь хотя бы есть честная альтернатива. Если не хотите, чтобы Windows увела папки в облако, ищите серую ссылку отказа и нажимайте её.
